  ...
  switch((wifi_uart_rx_buf[offset + CLUSTER] | (wifi_uart_rx_buf[offset + CLUSTER + 1] << 8))){
    case STD_CLUSTER:
      switch((wifi_uart_rx_buf[offset + DATA_START] | (wifi_uart_rx_buf[offset + DATA_START + 1] << 8))){
        case NUMBER_STATES:{
          if(wifi_uart_rx_buf[offset + DATA_START + 3] == 1)
            Key_get_home();
          else if(wifi_uart_rx_buf[offset + DATA_START + 3] == 2)
            Key_leave_home();
          else
            Key_sleep_mode();
  ...
  switch((wifi_uart_rx_buf[offset + CLUSTER] | (wifi_uart_rx_buf[offset + CLUSTER + 1] << 8))){
    ...
    case STD_CLUSTER:
      switch((wifi_uart_rx_buf[offset + DATA_START] | (wifi_uart_rx_buf[offset + DATA_START + 1] << 8))){
        case NUMBER_STATES:{
          uint8_t switch_t = 0;
          if(!(GPIO_ReadInputDataBit(GPIOB, KEY_LIGHT_1)))
            switch_t = 0x01;
          else if(!(GPIO_ReadInputDataBit(GPIOB, KEY_LIGHT_2)))
            switch_t = 0x02;
          else 
            switch_t = 0x03;
          memset(wifi_uart_tx_buf, 0, UART_RECV_DATA_LEN);
          memcpy(wifi_uart_tx_buf, (wifi_uart_rx_buf + offset), PROTOCOL_HEAD);
          Send_Ack_Data(&switch_t, 1, READ_ACK);
        }
        break;
      }
    default:
