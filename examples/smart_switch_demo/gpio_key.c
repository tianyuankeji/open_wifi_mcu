...
void mcu_send_led_status(uint8_t point, uint8_t led_status_tmp){
  unsigned char send_buff[15] = {0x5c,0xc5,0x0f,0x00,send_sn++,0x10,point,0x06,0x00,0x0a,0x00,0x00,0x01,led_status_tmp};
  send_buff[14] = get_check_sum(send_buff, 14);
  uart_send_string(send_buff,15);
}

/****************key1短按处理函数*******************/
static void Key1_Short_press_handle(void){
  uint8_t tmp_value = GPIO_ReadInputDataBit(GPIOB, KEY_LED1);
  if(!tmp_value)
    Key_led1_off();
  else
    Key_led1_on();
   mcu_send_led_status(1, tmp_value);
}
...
static void Key2_Short_press_handle(void){
  uint8_t tmp_value = GPIO_ReadInputDataBit(GPIOB, KEY_LED2);
  if(!tmp_value)
    Key_led2_off();
  else
    Key_led2_on();
  mcu_send_led_status(2, tmp_value);
}
...
uint8_t tmp_value = GPIO_ReadInputDataBit(GPIOA, KEY_LED3);
  if(!tmp_value)
    Key_led3_off();
  else
    Key_led3_on();
  mcu_send_led_status(3, tmp_value);
}
