...
void write_device_cluster_info(unsigned short offset){
  switch((wifi_uart_rx_buf[offset + CLUSTER] | (wifi_uart_rx_buf[offset + CLUSTER + 1] << 8))){
    case SWITCH_CLUSTER:
      GPIO_WriteBit(Light_mod[wifi_uart_rx_buf[offset + POINT] - 1].LED_PORT, 
                    Light_mod[wifi_uart_rx_buf[offset + POINT] - 1].LED_Pin,
                    (!wifi_uart_rx_buf[offset + DATA_START + 3]));
      memset(wifi_uart_tx_buf, 0, UART_RECV_DATA_LEN);
      memcpy(wifi_uart_tx_buf, wifi_uart_rx_buf + offset, FRAME_TYPE);
      Send_Ack_Data(wifi_uart_tx_buf, 1, WRITE_ACK);
      break;
    default:
      break;
  }
...
void read_device_cluster_info(unsigned short offset){
  switch((wifi_uart_rx_buf[offset + CLUSTER] | (wifi_uart_rx_buf[offset + CLUSTER + 1] << 8))){
    ...
    case SWITCH_CLUSTER:
      {
        uint8_t light_status = GPIO_ReadInputDataBit(Light_mod[(wifi_uart_rx_buf[offset + POINT] - 1)].LED_PORT, 
                                                     Light_mod[(wifi_uart_rx_buf[offset + POINT] - 1)].LED_Pin);
        memset(wifi_uart_tx_buf, 0, UART_RECV_DATA_LEN);
        memcpy(wifi_uart_tx_buf, (wifi_uart_rx_buf + offset), PROTOCOL_HEAD);
        light_status = !light_status;
        Send_Ack_Data(&light_status, sizeof(light_status), READ_ACK);    
      }
    break;
    default:
    break;
