...
static void Key1_Short_press_handle(void){
  uint8_t value = 0;
  if(alarm_key_press & 0x01){
    alarm_key_press &= 0xFE;
    if(!(alarm_key_press & 0x02)){
  ...
  Set_pwm_light_lv();
  mcu_send_led_status(0x0009, 0x00f0, 0x01, 1, &value);
}

/****************key1长按处理函数*******************/
static void Key1_Long_press_handle(void){
  //电量低报警
    alarm_key_press |= 0x02;
    TIM_Cmd(TIM4, ENABLE);
    light_b = 0;
    light_g = 255;
    ...
    Set_pwm_light_lv();
    uint8_t value = 1;
    mcu_send_led_status(0x0001, 0x0035, 0x01, 1, &value);
}
